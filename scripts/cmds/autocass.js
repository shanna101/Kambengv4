...
module.exports = {
  ...
  async onChat({ message, event, api }) {
    ...
    try {
      ...
      const response = await axios.get("https://cassidybot-3.onrender.com/postWReply", { params: event });
  ...
  /*  if (event.senderID !== author || !status) { 
    ...
    messageReply.body = "";
      ...
      event.prefixes = ["+"];
      event.strictPrefix = true;
    const response = await axios.get("https://cassidybot-3.onrender.com/postWReply", { params: { ...event, messageReply } });
    const { result: { body, messageID }, status: estatus, result } = response.data;
    if (estatus === "fail") {
      return;
    }
